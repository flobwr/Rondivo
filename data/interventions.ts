...
// Photos are not part of this record — they live in the shared photo store
// (data/documents/photos.ts), keyed by intervention id, so the same photos
// are visible from both the live intervention screen and the Photos module.
...
export type InterventionInput = Partial<Omit<Intervention, 'id'>>;

// Only interventions hardcoded in INTERVENTIONS can be durably edited — ids
// that fall back to a fresh `{ ...DEFAULT_INTERVENTION, id }` spread have
// nothing to write back to, same mock-data ceiling as `getIntervention`.
export function updateIntervention(id: string, patch: InterventionInput): Intervention | undefined {
  const existing = INTERVENTIONS[id];
